...
# keep salinity obs flagged 1 & 3 for Antigonish (Monk's Head) and Pictou (Pictou Landing). Other stations in these counties will be filtered to only keep salinity obs flagged 1 in the plot loop
...
ggplot() +
  ...
  ggsflabel::geom_sf_text_repel(
    ...
    force = force_fact, max.overlaps = 30, seed = map_seed ,
...
for(i in seq_along(waterbodies)){
  ...
  for(j in seq_along(stations)){
    ...
    if((county == "Antigonish" & station_j != "Monks Head") |
       (county == "Pictou" & station_j != "Pictou Landing")) {
